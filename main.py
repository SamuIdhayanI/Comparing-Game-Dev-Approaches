...
# importing enemy behaviour from own python class
...
# setting game name
pygame.display.set_caption("AI Siege Sentinel")

# these two lines make the game run in same speed in all systems
...
# here animation is just flipping the images fast
...
# create castle
castle = Castle(castle_img_100, castle_img_50, castle_img_25, SCREEN_WIDTH - 250, SCREEN_HEIGHT - 300, 0.2)  # 0.2 = 20%
...
while run:
    ...
    if game_over == False:
        ...
        # draw enemies
        enemy_group.update(screen, castle, bullet_group)  # giving the screen to display,
        # the castle's rectangle(as 'target') and the bullet group is also given
        ...
        if next_level == True:
            draw_text("LEVEL COMPLETE!", font_60, WHITE, 200, 200)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:  # (if the top right X is pressed)
            run = False
